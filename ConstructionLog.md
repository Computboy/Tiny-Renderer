...
教程中要求的平滑着色我已经实现了，现在的一步就是进行纹理坐标的映射

首先就是对obj文件中的vt行进行阅读，在f行中 b, e, h索引的是这个三角形三个顶点依次从1开始的vt行下标。

从vt行读到的数据是在**纹理贴图**中的坐标位置，直接获取颜色即可。
纹理贴图：可以指定例如obj模型文件中为`diffuse`的文件，但是最为正统的方式为：

- 在一系列顶点开头有`mtllib backpack.mtl`这样的标识符
- 于是在统计文件夹下找名为`backpack.mtl`的文件
- mtl文件中有所有参数，例如漫反射、镜面反射系数，以及所对应的纹理贴图文件

因此数媒中文件资源管理的思想也非常重要，实际上加载模型、读取纹理的方式有非常多，只是现代艺术家们构建的习惯或行业标准规定使然，使得`loadModel`整个流程变得规范化。

同样的，片段内部仍然需要线性插值。不必改变光栅化步骤，只是将base_color从纯白转变为了从纹理贴图中插值读出的颜色值。

> 这里暂时不用管，切线空间是在Displacement-Mapping和SSAO等渲染技术下才会用到的。
同时，法线贴图沿用了uv坐标，三维向量映射是$[0,255] \to [-1,1]$，切线空间的法线坐标。注意BGRA格式存储。
