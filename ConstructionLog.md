...
## Day7 - Blinn-Phong 光照与着色

Day4 的 Z-Buffer 解决了「谁挡住谁」的问题，Day5 的 MVP 管线解决了「顶点在哪里」的问题，Day6 的 Shader 重构解决了「代码怎么组织」的问题。但在视觉效果上，模型仍然只是随机颜色填充——没有任何立体感。Day7 的目标是引入光照计算，让模型呈现出明暗变化，使画面产生真正的三维立体感。

---

### 1. Blinn-Phong 光照模型

Blinn-Phong 是 Phong 光照模型的改进版本，它将物体表面接收到的光分解为三个独立分量：

| 分量 | 物理含义 | 视觉表现 |
| ---- | -------- | -------- |
| **环境光 (Ambient)** | 场景中无处不在的间接光照，不依赖光源方向 | 均匀的基底亮度，防止暗面全黑 |
| **漫反射 (Diffuse)** | 光线被粗糙表面向各个方向均匀散射 | 表面亮度随光照角度变化，但不依赖观察方向 |
| **镜面反射 (Specular)** | 光线在光滑表面上的集中反射 | 高光斑点，依赖观察方向 |
...
$$
I_{\text{ambient}} = k_a \cdot \text{baseColor}
$$

其中 $k_a$ 是环境光强度（本项目取 $0.10$），确保暗面不会完全漆黑。

#### 2.2 漫反射 — Lambert 余弦定律

漫反射的核心规律是 **Lambert's Cosine Law**：表面接收到的光通量与光线方向和表面法线夹角的余弦成正比。

对于着色点 $P$：
- $\vec{N}$ = 表面单位法线
- $\vec{L}$ = 指向光源的单位方向

夹角 $\theta$ 越大（光线越斜），单位面积接收到的光通量越少：

$$
I_{\text{diffuse}} = k_d \cdot \text{lightColor} \cdot \max(0, \vec{N} \cdot \vec{L})
$$

> $\max(0, \cdot)$ 将负值截断为 0——当法线背对光源时（$\vec{N} \cdot \vec{L} < 0$），该面不接收漫反射光。此处使用 `cwiseproduct`（逐分量乘法）将光源颜色与漫反射系数混合。

#### 2.3 镜面反射 — Blinn 的半程向量优化

原始 Phong 模型通过反射向量 $\vec{R}$ 与观察方向 $\vec{V}$ 的夹角计算高光：
...
$\vec{H}$ 是光源方向与观察方向的中间方向。$\vec{N}$ 与 $\vec{H}$ 越接近，意味着 $\vec{R}$ 与 $\vec{V}$ 的夹角越小——高光越亮。因此：

$$
I_{\text{specular}}^{\text{Blinn}} = k_s \cdot \text{lightColor} \cdot \max(0, \vec{N} \cdot \vec{H})^{n_s}
$$

> Blinn-Phong 的优势：半程向量的计算比反射向量更轻量；且当光源和相机都在远处时，$\vec{H}$ 近似常量，可以预先计算。

参数含义：

| 参数 | 含义 | 本项目取值 |
| ---- | ---- | ---------- |
| $k_s$ | 镜面反射强度（specularStrength） | $0.45$ |
| $n_s$ | 高光指数 / 反光度（shininess） | $32$ |

- $n_s$ 越大，高光范围越集中（表面越光滑）；
- $n_s$ 越小，高光范围越扩散（表面越粗糙）。
...
最终将各分量结果 clamp 到 $[0, 1]$，转换为像素颜色写入帧缓冲。

---

### 3. 法线数据：扩展 OBJ 解析器

光照计算需要每个顶点的法线向量。OBJ 文件中以 `vn` 开头的行为法线数据：

```obj
vn 0.0000 1.0000 0.0000
```

解析代码（`tinyobjloader.cpp`）新增：
...
同时面索引 `f` 的第三个字段就是法线索引，已有的 `parseFaceVertex` 已经解析了它（`idx.z`），因此无需修改面解析逻辑。

头文件（`tinyobjloader.h`）新增：

- `std::vector<normal3f> normals_` —— 存储法线数组；
- `const normal3f normal(int faceIndex, int normalIndex) const` —— 便捷访问方法。
...
`normal3f` 是 `geometry.h` 中新增的类型别名（与 `vec3f` 同构，语义上区分）：
...
### 4. `Blinn_PhongShader`：将光照模型映射到 Shader 接口

得益于 Day6 的 Shader 抽象，Day7 的所有光照逻辑只需要写一个新的 `IShader` 子类，核心渲染管线完全不变。
...
Blinn_PhongShader(
    const Model& mesh_,
    const mat4f& modelMatrix, const mat4f& viewMatrix, const mat4f& projectionMatrix,
    const color3f& lightColor_,     // 光源颜色
    const vec3f& lightPosition_,    // 光源位置（世界空间）
    const vec3f& cameraPos_         // 相机位置（世界空间）
);
```
...
- 顶点位置仅经过 **Model** 变换到达世界空间——不经过 View 和 Projection——存入 `varying_worldPos`，供 fragment 阶段进行光照计算。因为光源和相机都在世界空间中，在世界空间中算光照明最自然；
- 法线同理保留在模型空间（经过 `normalize`）存入 `varying_normal`；
- MVP 变换结果存入 `gl_Position`，返回给 `Draw()` 做透视除法和视口变换——与 Day6 完全相同。

> 当前实现暂不考虑非均匀缩放（Non-uniform Scaling）对法线的影响。若 Model 矩阵包含非均匀缩放，法线应当乘以 Model 矩阵的逆转置矩阵 $\mathbf{(M^{-1})^\top}$。
...
std::pair<bool, TGAColor> fragment(const vec3f& bar) const override {
    // 1. 重心坐标插值
    vec3f frag_WorldPos = varying_worldPos[0] * bar.x
                        + varying_worldPos[1] * bar.y
                        + varying_worldPos[2] * bar.z;
    normal3f frag_Normal = (varying_normal[0] * bar.x
...
整个 fragment 阶段完整实现了 Blinn-Phong 的三个光照分量，并通过重心坐标将世界坐标和法线从三角形顶点平滑插值到每个像素。
...
Blinn_PhongShader myShader(
    ...
    vec3f(2.0f, -4.0f, 6.0f),  // 点光源位置（右-上-前方）
    CamPos                      // 相机即观察者
);
...
只需将 `FlatShader` 替换为 `Blinn_PhongShader`，传入光源颜色、光源位置和相机位置即可。`Draw()` 和 `Rasterization()` 无需任何改动——这正是 Day6 重构带来的架构收益。
...
从渲染结果可以观察到 Blinn-Phong 光照模型的所有关键效果：

- **明暗变化**：面向光源的面（漫反射分量 $\vec{N} \cdot \vec{L}$ 大）更亮，背向光源的面更暗，产生自然的立体感；
- **高光**：在光源和观察方向的中分面附近（$\vec{N}$ 接近 $\vec{H}$），出现明显的镜面高光斑点；
- **环境光基底**：背光面仍然保留 $10\%$ 的常亮底色，避免完全漆黑；
- **平滑着色**：由于重心坐标逐像素插值法线，三角形内部颜色平滑过渡，不存在 Day3 那种面与面之间的生硬边界。
...
| 文件 | 变更 |
| ---- | ---- |
| `shader.h` | **新增** `Blinn_PhongShader` 类 —— 世界空间 Blinn-Phong 光照，点光源 |
| `tinyobjloader.h` | 新增 `normals_` 成员 + `normal()` 便捷方法 |
| `tinyobjloader.cpp` | 新增 `vn` 行解析逻辑 + `normal()` 实现 |
| `geometry.h` | 新增 `normal3f` 和 `color3f` 类型别名 |
| `main.cpp` | 切换为 `Blinn_PhongShader`，传入光源参数 |
...
```text
OBJ 文件 → 顶点(位置+法线) + 面索引(v/vt/vn)
         → [Model] 世界空间 — 同时保存 worldPos 与 normal（用于光照）
         → [View] 摄像机空间
         → [Perspective] 裁剪空间 → [÷w] NDC → [Viewport] 屏幕空间
         → 包围盒计算 → 叉乘半平面判定
         → 重心坐标插值 (worldPos + normal)
         → Blinn-Phong 光照计算 (ambient + diffuse + specular)
         → Z-Buffer 深度测试 → 像素填充 → 图像输出
```

Day7 完成后，渲染器首次输出了带有光照和明暗效果的图像。模型表面的每个像素颜色都由光照物理模型计算得出，实现了从「随机颜色填充的几何体」到「被真实点光源照亮的立体物体」的跨越。下一步将引入纹理映射，使模型表面呈现丰富的材质细节。
...
加入阴影映射后，画面的空间真实感有了质的提升——背包在地面上投下了方向明确的阴影，光源衰减使远离光源的地面区域自然变暗。以下是阴影渲染前后的对比：

<div align="center">
  <img src="attachments/阴影渲染前后对比.png" width="780">
</div>
...
- **阴影投影**：背包在地面平面上投下了符合光源方向的阴影，地面不再是均匀受光的平面；
- **光照衰减**：远离点光源的地面边缘亮度自然下降，画面不再「全场景等亮」；
- **纹理 + 法线 + 阴影协同**：背包表面的 diffuse 纹理提供颜色、normal map 提供凹凸细节、shadow map 提供空间遮挡——三种贴图各司其职，叠加后产生接近离线渲染的画面质感；
- **Alpha-Testing 生效**：背包的透明区域（如边缘细节）被正确丢弃，不会遮挡阴影或背景。
...
│ OBJ → [Model Matrix] → [Camera View Matrix] → [Camera Projection Matrix] → [Perspective Divide by w]     │
│     → NDC → [Viewport Transform]                                                                         │
│     → Rasterization → Camera Depth Buffer (SSAO Lookup Resource)                                         │
└──────────────────────────────────────────────────────────────────────────────────────────────────────────┘
                            │
┌─ Pass 3: Camera Final Shading Pass ──────────────────────────────────────────────────────────────────────┐
│ OBJ → [Model Matrix] → [Camera View Matrix] → [Camera Projection Matrix]                                 │
│        → [Perspective Divide by w] → NDC → [Viewport Transform]                                          │
...
<div style="max-width:620px;margin:22px auto;padding:16px 24px;border:3px double #475569;border-radius:6px;box-shadow:inset 0 0 0 1px #cbd5e1;background:#f8fafc;text-align:center;font-size:17px;color:#0f172a;">
超出教程范围的软光栅渲染效果优化流程安排
</div>

> #### Day12 抗锯齿
> - 先渲染至尺寸`2×width、2×height`的离屏帧缓存，再将单个屏幕像素对应的4个超采样像素求取均值完成降采样，重点修复模型轮廓、地表边界产生的锯齿失真。
>   - 底层原理等价于在像素内部布置多采样点位，放大分辨率是工程落地的简化实现思路；
>   - 开启线框渲染（Wireframe-Draw）能够直观对比优化前后的边缘走样变化。
>
> #### Day13 Shadow-Mapping优化
> - 依托视角动态修正阴影Bias偏移值，搭配PCF纹理滤波算法实现阴影边缘柔化，改善阴影漏光、硬边锯齿两大通病。
>   - Bias随视线与表面法线夹角动态调整，规避近平面阴影痤疮与远距离飘移漏洞；
>   - PCF通过阴影贴图邻域采样取平均，把生硬的阴影边界过渡为自然的软阴影。
>
> #### Day14 SSAO优化
> - 在基础随机采样生成AO贴图后，使用多级模糊滤波对画面颗粒噪点做平滑处理，削弱采样离散带来的斑驳杂色问题。
>   - 原理是利用邻域像素信息抵消随机采样的离散误差，低采样率下也能获得干净的环境遮蔽效果；
>   - 分步高斯模糊便于分步调试，直观观察降噪前后AO细节变化。
>
> #### Day15及以后——光线追踪（Ray Tracing）
> - 基于软光栅现有框架接入光线追踪管线，逐像素发射光路做相交测试与光照迭代计算，实现物理精准的反射、折射与全局光照效果。
>   - 相较于传统光栅的近似光照模型，光线追踪依托光路追踪还原真实光学传播规律，是本项目开发重难点；
>   - 可分步实现基础投射光线、阴影光线、反射光线，逐步完善整套追踪逻辑。


## Day12*- 抗锯齿
### 背景介绍

MSAA 是一种直观的硬件友好型抗锯齿方案，通过在单个像素区域内布置多个子采样点，对三角形覆盖的子样本逐个做光栅化判定，最后用有效采样的颜色加权平均输出像素色，从采样层面补齐高频边缘的采样密度，修复画面变化频率过快、像素原生采样频率不足引发的混叠锯齿。

SSAA（超级采样抗锯齿）是最朴素的全域超采样方案：先把渲染分辨率整体成倍放大，在高分辨率帧缓冲区完成全画面光栅采样，最后向下滤波缩放到目标屏幕分辨率；依靠全局超高采样率一次性抑制全图所有高频细节混叠，抗锯齿效果理论最优，但全像素超额采样带来巨大算力开销。

> 当前软光栅环境下，决定先实现SSAA渲染流程，虽然会带来明显的性能开销问题，但由于结果只是渲染一张静态图，只需保证最终渲染质量即可。

撰写支持可调倍率的超采样函数
```cpp
TGAImage ResolveSSAA(const TGAImage& highRes, int width, int height, int scale)
```
需要更改`perspectiveMatrix`, `ViewportMatrix`等一系列采样尺径。

### SSAA 抗锯齿效果对比

左图为未开启抗锯齿时的渲染结果（局部放大展示），可以看到斧头刀刃、木柄边缘以及背包轮廓处存在明显的阶梯状锯齿。右图开启了 3×3 Supersampling Anti-Aliasing：渲染器首先将场景绘制到高分辨率 framebuffer 中，再通过 box filter 将每个 3×3 子像素块平均合成为最终图像中的一个像素。

可以看到，模型轮廓和斜向边缘的锯齿得到了明显缓解，整体画面更加稳定、柔和。代价是部分高频纹理细节会被轻微平滑。

<div align="center">
  <img src="attachments/局部细节对比.png" width="600">
</div>